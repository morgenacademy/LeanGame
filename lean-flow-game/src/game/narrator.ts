...
export function narrate(g: GameState): NarratorLine | null {
  ...
  if (g.mode === 'push') {
    ...
    if (built === 0 && !holding && d >= 1)
      return line('p-grab', 'Daar is een set! Pak ’m — klik op het vak of druk spatie.', 'guide');
    if (built === 0 && holding)
      return line('p-place', 'Top. Plaats nu steen voor steen — klik of spatie — tot het huis af is.', 'guide');
  ...
  if (holding) return line('q-place', 'Bouwen maar — dit huis is verkocht zodra het af is.', 'guide');
  if (!holding && d >= 1) return line('q-grab', 'Er ligt een set klaar in de juiste kleur. Pak ’m.', 'guide');
