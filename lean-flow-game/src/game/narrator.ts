...
export function narrate(g: GameState): NarratorLine | null {
  ...
  if (g.mode === 'push') {
    ...
    if (built === 0 && holding)
      return line('p-place', 'Bouw dit huis: sleep de juiste kleur uit de bak omhoog naar de bouwtekening — steen voor steen.', 'guide');
  ...
  if (holding) return line('q-place', 'Sleep de juiste kleur omhoog — dit huis is verkocht zodra het af is.', 'guide');
